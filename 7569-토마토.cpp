...
int di[6] = { -1,0,1,0, 0, 0 }; //상우하좌위아래 움직일 때 행의 변위
int dj[6] = { 0,1,0,-1, 0,0 }; //상우하좌위아래 움직일 때 열의 변위
int dk[6] = { 0, 0, 0, 0 , 1, -1 }; //상우하좌위아래 움직일 때 층의 변위
...
void bfs(void)
{
	...
	{
		int curk = get<0>(q.front()); //꺼낸 토마토의 층 인덱스
		int curi = get<1>(q.front()); //꺼낸 토마토의 행 인덱스
		int curj = get<2>(q.front()); //꺼낸 토마토의 열 인덱스
		visited[curk][curi][curj] = true; //해당 토마토 방문표시
		q.pop(); //큐 가장 앞에 있던 토마토 꺼내기
		qControlSize--; //같은 날짜에 익어있던 토마토 개수 -1 해주기-> 이 값이 0이되면 같은 날짜에 익은 토마토를 다 꺼낸 것이므로 main함수로 갈 수 있도록함

		for (int i = 0; i < 6; i++) //상우하좌위아래 돌아가며 익을 수 있는 토마토가 있는지 확인
		{
			int nexti = curi + di[i];
			int nextj = curj + dj[i];
			int nextk = curk + dk[i];

			if (nexti >= 0 && nexti < M && nextj >= 0 && nextj < N && nextk >= 0 && nextk < H && visited[nextk][nexti][nextj] == false && map[nextk][nexti][nextj] == 0) //범위 안에 있는 토마토이고, 토마토가 이미 방문안됐고, 토마토가 안익었다면
			{
				visited[nextk][nexti][nextj] = true; //토마토 방문표시
				map[nextk][nexti][nextj] = 1; //익을 예정임을 표시
				q.push(make_tuple(nextk, nexti, nextj));; //다음에 이함수가 호출됐을 때 이 익은 토마토에서 다시 bfs해봐야하므로 큐에 push
	...
	for (int k = 0; k < H; k++)
	{
		for (int i = 0; i < M; i++)
		{
			for (int j = 0; j < N; j++)
			{
				...
				cin >> temp;
				map[k][i][j] = temp;
				if (map[k][i][j] == 1) //처음부터 익어있던 토마토는 q에 push해서 bfs함수 호출 시 바로 이 토마토들부터 너비우선탐색 할 수 있도록 함
				{
